...
class Spxw7dteHardExit(QCAlgorithm):
    STRATEGY_NAME = "hard_exit"
    ACTION = "close"
    ...
    def monitor_position(self):
        ...
        if not self.repair_manager.is_pending:
            self.repair_manager.start(trigger["reason"])
    ...
    def on_order_event(self, order_event):
        if self.repair_manager.handle_order_event(order_event):
