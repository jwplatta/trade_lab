...
class Spxw7dteRepairExperiment(QCAlgorithm):
    ...
    EXPERIMENT_NAME = "exp08_close_late_midday"
    ACTION = "close"
    TRIGGER_MODE = "delta_or_breach"
    ...
    def repair_trigger_reason(self):
        ...
        call_delta = self.short_leg_delta(chain, self.trade["short_call"])
        put_delta = self.short_leg_delta(chain, self.trade["short_put"])
    ...
    def short_leg_delta(self, chain, symbol):
        contract = next((contract for contract in chain if contract.symbol == symbol), None)
        if contract is None:
            return None
